...
void sqrtm(const double A[9], creal_T X[9])
{
  ...
  for (ia = 0; ia < 9; ia++) {
    if (p) {
      U_re_tmp = A[ia];
      if (std::isinf(U_re_tmp) || std::isnan(U_re_tmp)) {
        p = false;
      }
    } else {
      p = false;
  ...
  if (!p) {
    ...
    internal::lapack::xgehrd(b_A, tau);
    std::copy(&b_A[0], &b_A[9], &Vr[0]);
    for (j = 1; j >= 0; j--) {
      ia = (j + 1) * 3;
      for (b_i = 0; b_i <= j; b_i++) {
        Vr[ia + b_i] = 0.0;
    ...
    for (b_i = 1; b_i >= 0; b_i--) {
      ...
      if (b_i + 1 < 2) {
        ...
        if (lastv > 0) {
          internal::blas::xgemv(lastv, lastc, Vr, iaii + 3, Vr, iaii, work);
          internal::blas::xgerc(lastv, lastc, -tau[b_i], iaii, work, Vr,
                                iaii + 3);
        }
        lastv = iaii + 1;
    ...
    for (iaii = 1; iaii >= 0; iaii--) {
      ...
      if (U_re_tmp != 0.0) {
        ...
        d_tmp = b_A[lastc + 1];
        d = d_tmp;
        mu1_re = internal::reflapack::xdlanv2(r, t1_im, c, d, rt1i, mu1_im, s,
                                              cs, sn);
        mu1_re -= d_tmp;
      ...
      do {
        ...
        if (b_i <= j) {
          ...
          if ((T[lastc].re != 0.0) || (T[lastc].im != 0.0)) {
            std::memset(&R[0], 0, 9U * sizeof(creal_T));
            for (j = 0; j < 3; j++) {
              iaii = j + 3 * j;
              R[iaii] = T[iaii];
              b_sqrt(R[iaii]);
  ...
  exitg3 = false;
  while ((!exitg3) && (j < 3)) {
    s = (std::abs(b_A[3 * j]) + std::abs(b_A[3 * j + 1])) +
        std::abs(b_A[3 * j + 2]);
    if (std::isnan(s)) {
      r = rtNaN;
      exitg3 = true;
...
// End of code generation (sqrtm.cpp)
